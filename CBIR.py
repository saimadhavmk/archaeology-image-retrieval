...
def cosineBasedimageRetrieval(query, vgg_classifier): #cosine based image similarity calculation 
    ...
    for key in list(dataset.keys()):
        dico = {}
        img = dataset[key]
        cosine_similarity = dot(img, query)/(norm(img)*norm(query)) #np.linalg.norm(np.array(query) - np.array(img))
...
def cbir():
    ...
    temp = []
    start = len(list_images) - 1
    while start >= 0:
        temp.append(list_images[start])
        start -= 1
        if len(temp) >= 10:
            break
    list1 = []
...
font1 = ('times', 13, 'bold')
upload = Button(main, text="Upload Archeology Dataset", command=upload)
